...
class ReportService
{
    ...
    public function getReportsData(array $filters): array
    {
        ...
        // Ambil versi cache aktif untuk tenant ini (Cache Buster) guna sinkronisasi real-time
        $cacheVersion = \Illuminate\Support\Facades\Cache::get("reports_cache_version_{$tenantId}", 1);
        $cacheKey = "reports_data_{$tenantId}_{$branchId}_{$startDate}_{$endDate}_p{$page}_pp{$perPage}_v{$cacheVersion}";

        return \Illuminate\Support\Facades\Cache::remember($cacheKey, now()->addMinutes(3), function () use ($filters, $branchId, $startDate, $endDate, $branches, $page, $perPage) {
            ...
            $currentBranch = $user ? Branch::find($user->branch_id) : null;
