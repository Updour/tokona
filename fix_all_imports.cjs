...
// Run tsc to get the exact errors
try {
    ...
    for (const line of lines) {
        if (line.includes('error TS2304: Cannot find name \'formatNumber\'')) {
            const match = line.match(/(resources\/js\/[^:]+):/);
            if (match) filesToFix.add(match[1]);
        }
        if (line.includes('error TS2304: Cannot find name \'formatDateTime\'')) {
            const match = line.match(/(resources\/js\/[^:]+):/);
            if (match) filesToFix.add(match[1]);
        }
    }
